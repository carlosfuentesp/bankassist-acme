...
- `src/documents`, `src/retrieval`: ingestión documental y búsqueda híbrida.
- `src/agents/bankassist`: orquestador, reglas, guardrails, costes y entry points.
- `src/app`: chatbot para Databricks Apps.
- `evaluation`: golden dataset legible para revisión.
- `resources`: pipelines, jobs, evaluación, app y workflow end-to-end como DAB.
...
5. Para ejecutar las pruebas y evaluación local:
    ```
    $ uv sync --dev
    $ uv run pytest
    $ uv run bankassist-evaluate
    ```
...
Las tarifas para el estimador se configuran con
`BANKASSIST_MODEL_INPUT_USD_PER_MILLION`,
`BANKASSIST_MODEL_OUTPUT_USD_PER_MILLION`, `BANKASSIST_GENIE_USD_PER_QUERY` y
`BANKASSIST_AI_SEARCH_USD_PER_QUERY`. Sin valores confirmados, la aplicación
muestra cero y lo identifica como una estimación no configurada.
