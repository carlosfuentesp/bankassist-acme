...
from agents.bankassist.costs import estimate_cost
...
def _trace_agent(function: Callable) -> Callable:
    try:
        import mlflow
    except ImportError:
        return function
    return mlflow.trace(name="ask_bankassist", span_type="AGENT")(function)
...
async def ask_bankassist(
    ...
    answer, usage = synthesize_bankassist_answer(question, evidence, assessments)
    cost = estimate_cost(**usage).to_dict()
